...
public class FuseRandomAccessStream implements FuseFileStream {
  ...
  public FuseRandomAccessStream(FileSystem fileSystem, AlluxioURI uri) {
    try {
      ...
      File tmp = File.createTempFile("alluxio-fuse-random-access", null);
      System.out.println("yimin file " + tmp);
      ...
      out.close();
      mRandomAccessFile = new RandomAccessFile(tmp, "rw");
      mURI = uri;
      is.close();
    } catch (Exception e) {
      e.printStackTrace();
  ...
  public void close() {
    try {
      // TODO -> rename it to a tmp file before deletion
